...
async function main() {
    ...
    const subjectTemplate = getArgValue('--subject', 'איך מפסיקים את "מה קורה עם התיק שלי" במשרד — Melamedia');
...
<div dir="rtl" style="font-family: Arial, sans-serif; line-height: 1.75; color: #222; font-size: 0.9375rem;">
    ...
    <p>קוראים לי לירוי מחברת <strong>Melamedia</strong>, ואנחנו מפתחים פתרונות טכנולוגיים לעסקים.</p>

    <p>
        בנינו פלטפורמה למשרדי עו"ד שכבר עובדת בכמה משרדים בארץ. המטרה שלה פשוטה:
        להעלים את כל ה"רגע אני בודק", "תכף מתעדכן", "מה קורה עם התיק שלי?" ולרכז את העבודה במקום אחד מסודר ונוח.
    </p>

    <p><strong>איך זה עובד בפועל במשרד:</strong></p>
    <ul>
        <li>הלקוח מקבל אפליקציה / אזור אישי משלו ורואה בדיוק באיזה שלב התיק עומד, אילו מסמכים קיימים, מה הוגש ומתי.</li>
        <li>ברגע שמישהו במשרד מעדכן שלב, הלקוח מקבל אוטומטית הודעה ב-SMS, מייל והתראה באפליקציה, בלי לזכור להתקשר ידנית.</li>
        <li>חתימות דיגיטליות עם OTP: שולחים מסמך, הלקוח חותם מהטלפון, והכל נשמר בתיק עם תיעוד מלא.</li>
        <li>תזכורות אוטומטיות לפגישות, מסמכים חסרים וחתימות שצריכות להתבצע.</li>
        <li>הכל מותאם אישית למשרד שלך: לוגו, צבעים ושם המשרד. הלקוח לא רואה את Melamedia, הוא רואה את [[firm]].</li>
    </ul>

    <p>
        השורה התחתונה: פחות בזבוז זמן על טלפונים וניהול אדמיניסטרטיבי, ויותר עבודה מסודרת במשרד עם לקוחות מעודכנים בזמן אמת.
    </p>

    <p>
        חשוב לי להדגיש: המערכת לא באה להחליף את המערכת הקיימת במשרד, אלא להיות פלטפורמת עזר שמחזקת אותה.
        אני אומר את זה כי מניסיון, גם משרדים טכנופובים שהתחילו לתקופת ניסיון אמרו לי בסוף "מזל שהתחלנו לעבוד ביחד".
    </p>

    [[sales_deck_paragraph]]

    <p>
        אם זה נשמע רלוונטי, אשמח ל-15 דקות בזום או בטלפון כדי להראות לך איך זה נראה בפועל.
        אפשר פשוט להשיב למייל הזה עם זמן שנוח לך.
    </p>
