#!/usr/bin/env bash
# sync-prod-to-ashrafessa-neon.sh — Sync AshrafEssa Neon DB from AshrafEssa production VPS
# Usage: bash scripts/sync-prod-to-ashrafessa-neon.sh
...
DUMP_REMOTE="/tmp/ashrafessa_prod.sql"
DUMP_LOCAL="$SCRIPT_DIR/../ashrafessa_prod.sql"
DUMP_CLEAN="$SCRIPT_DIR/../ashrafessa_clean.sql"

echo ""
echo "=== [1/7] Dumping AshrafEssa production DB (read-only) ==="
...
"$PSQL" "$NEON_DIRECT_URL" -A -t -c "
  ...
  SELECT 'company=' || setting_value FROM platform_settings WHERE category='firm' AND setting_key='COMPANY_NAME' LIMIT 1;
"
...
echo ""
echo "=== Done! AshrafEssa Neon DB synced from AshrafEssa production ==="
