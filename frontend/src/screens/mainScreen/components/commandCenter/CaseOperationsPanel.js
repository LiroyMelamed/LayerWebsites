                ...
                <>
                        ...
                        {managerWorkload.length === 0 ? (
                            ...
                            managerWorkload.map((m) => (
                                <WorkloadBar
                                    ...
                                    sublabel={
                                        m.urgentCases > 0
                                            ? t("managerHome.operations.urgentShort", {
                                                count: m.urgentCases,
                                            })
                                            : m.needsAttention > 0
                                                ? t("managerHome.operations.needsAttentionShort", {
                                                    count: m.needsAttention,
                                                })
                                                : null
                                    }
                                    onClick={() => {
                    ...
                    {unassignedCases?.count > 0 && (
                        <SimpleContainer
                            ...
                            onClick={() => navigate(`${AdminStackName}${AllCasesScreenName}?status=open`)}
                        >
                    ...
                    {topStages.length > 0 && (
                        <>
                            ...
                            <SimpleContainer className="lw-commandCenter__stageGrid">
                                {topStages.map((s) => (
                                    <SimpleContainer key={`${s.case_type}-${s.stage}`} className="lw-commandCenter__stageChip">
